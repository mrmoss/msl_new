...
int main()
{
	...
	if(!encrypt_rsa(plaintext,publickey,encrypted_text))
	{
		std::cout<<"could not encrypt data!"<<std::endl;
		return 0;
	}

	if(!decrypt_rsa(encrypted_text,privatekey,decrypted_text))
	{
	...
	if(!encrypt_aes(plaintext,key,iv,encrypted_text))
	{
		std::cout<<"could not encrypt data!"<<std::endl;
		return 0;
	}

	if(!decrypt_aes(encrypted_text,key,iv,decrypted_text))
	{
		std::cout<<"could not decrypt data!"<<std::endl;
		return 0;
	}
